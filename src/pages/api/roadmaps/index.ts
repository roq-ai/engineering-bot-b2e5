...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createRoadmap() {
    ...
    const body = { ...req.body };
    if (body?.mentor_feedback?.length > 0) {
      const create_mentor_feedback = body.mentor_feedback;
      body.mentor_feedback = {
        create: create_mentor_feedback,
      };
    } else {
      delete body.mentor_feedback;
    }
