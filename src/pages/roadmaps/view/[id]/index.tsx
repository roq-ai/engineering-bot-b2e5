...
import { getRoadmapById } from 'apiSdk/roadmaps';
import { Error } from 'components/error';
import { RoadmapInterface } from 'interfaces/roadmap';
import useSWR from 'swr';
import { useRouter } from 'next/router';
import { AccessOperationEnum, AccessServiceEnum, useAuthorizationApi, withAuthorization } from '@roq/nextjs';
import { deleteMentorFeedbackById } from 'apiSdk/mentor-feedbacks';
...
function RoadmapViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<RoadmapInterface>(
    ...
    () =>
      getRoadmapById(id, {
        relations: ['user', 'mentor_feedback', 'student_progress'],
      }),
  );

  const mentor_feedbackHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteMentorFeedbackById(id);
      await mutate();
    } catch (error) {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('roadmap', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/roadmaps/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('mentor_feedback', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/mentor-feedbacks/create?roadmap_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.mentor_feedback?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/mentor-feedbacks/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.feedback}</Td>
                            <Td>
                              {hasAccess('mentor_feedback', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/mentor-feedbacks/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('mentor_feedback', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    mentor_feedbackHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('student_progress', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/student-progresses/create?roadmap_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
